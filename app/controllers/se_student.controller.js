const SeStudent = require("../models/se_student.model.js");
...
exports.create = (req, res) => {
  ...
  const se_student = new SeStudent({
  ...
  SeStudent.create(se_student, (err, data) => {
    if (err)
      res.status(500).send({
        message:
...
exports.findOne = (req, res) => {
  SeStudent.findByEmail(req.params.email, (err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  ...
  SeStudent.updateByEmail(
    req.params.email,
    new SeStudent(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
...
exports.delete = (req, res) => {
  SeStudent.remove(req.params.studentId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
